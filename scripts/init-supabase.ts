...
async function initSupabaseDatabase() {
  try {
    ...
    for (const enumSql of enums) {
      const { error } = await supabase.rpc("exec", { query: enumSql })
      if (error) {
        console.error(`Error al crear enum: ${error.message}`)
      }
    }
    ...
    for (const tableSql of tables) {
      const { error } = await supabase.rpc("exec", { query: tableSql })
      if (error) {
    ...
    console.error("Error al inicializar la base de datos en Supabase:", error)
  ...
  initSupabaseDatabase()
    .then((result) => {
      if (result.success) {
        ...
        console.error("Error al inicializar la base de datos en Supabase:", result.error)
        process.exit(1)
      }
    })
    .catch((error) => {
...
export default initSupabaseDatabase
